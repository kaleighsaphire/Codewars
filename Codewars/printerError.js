...
P: given a string of lower case letter
R: return string of number of appearances of letters n-z over total length
E: s="aaabbbbhaijjjm"
printer_error(s) => "0/14"

s="aaaxbbbbyyhwawiwjjjwwm"
printer_error(s) => "8/22"
PC: split, filter, match a-m, join, return s-length - new length over s-length 
*/

function printerError(s) {
    let safeLetters = s.split("").filter(e => e.match(/[a-m]/g)).join("")
    return `${s.length - safeLetters.length}/${s.length}`
}